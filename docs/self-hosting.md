...
2. **Remote/private access**: bind to `0.0.0.0` only behind built-in auth, Cloudflare Access,
   Tailscale, a VPN, or another access-controlled proxy.
...
This built-in auth is intentionally simple. For a public hostname, prefer a real access layer such
as Cloudflare Access, Tailscale, VPN auth, Caddy/nginx auth, or your organization's SSO. You can use
both built-in auth and a proxy-level access policy if you want defense in depth.
